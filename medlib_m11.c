...
{
	TERN_m11			use_default_channel, use_global_name;
	si8				i, n_chans;
	CHANNEL_m11			*chan;
	...
	if (channel == NULL) {
		use_global_name = FALSE_m11;
		if (channel_name == NULL)
			use_global_name = TRUE_m11;
		else if (*channel_name == 0)
			use_global_name = TRUE_m11;
		if (use_global_name == TRUE_m11) {
			if (*globals_m11->reference_channel_name)
				channel_name = globals_m11->reference_channel_name;
			else
