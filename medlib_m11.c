...
SEGMENT_m11	*allocate_segment_m11(SEGMENT_m11 *seg, FILE_PROCESSING_STRUCT_m11 *proto_fps, si1 *enclosing_path, si1 *chan_name, ui4 type_code, si4 seg_num, TERN_m11 seg_recs)
{
	...
	switch (type_code) {
		case TIME_SERIES_CHANNEL_TYPE_m11:
			...
			seg->metadata_fps = FPS_allocate_processing_struct_m11(NULL, NULL, TIME_SERIES_METADATA_FILE_TYPE_CODE_m11, METADATA_BYTES_m11, proto_fps, METADATA_BYTES_m11);
			uh = seg->metadata_fps->universal_header;
			if (uh->segment_UID == UID_NO_ENTRY_m11)
				generate_UID_m11(&uh->segment_UID);
			uh->segment_number = seg_num;
			snprintf_m11(seg->path, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", enclosing_path, seg->name, TIME_SERIES_SEGMENT_DIRECTORY_TYPE_STRING_m11);
			snprintf_m11(seg->metadata_fps->full_file_name, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", seg->path, seg->name, TIME_SERIES_METADATA_FILE_TYPE_STRING_m11);
			// time series data
			seg->time_series_data_fps = FPS_allocate_processing_struct_m11(NULL, NULL, TIME_SERIES_DATA_FILE_TYPE_CODE_m11, 0, seg->metadata_fps, 0);
			snprintf_m11(seg->time_series_data_fps->full_file_name, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", seg->path, seg->name, TIME_SERIES_DATA_FILE_TYPE_STRING_m11);
			// time series indices
			seg->time_series_indices_fps = FPS_allocate_processing_struct_m11(NULL, NULL, TIME_SERIES_INDICES_FILE_TYPE_CODE_m11, TIME_SERIES_INDEX_BYTES_m11, seg->metadata_fps, 0);
			snprintf_m11(seg->time_series_indices_fps->full_file_name, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", seg->path, seg->name, TIME_SERIES_INDICES_FILE_TYPE_STRING_m11);
			break;
		case VIDEO_CHANNEL_TYPE_m11:
			// metadata: used as prototype
			seg->metadata_fps = FPS_allocate_processing_struct_m11(NULL, NULL, VIDEO_METADATA_FILE_TYPE_CODE_m11, METADATA_BYTES_m11, proto_fps, METADATA_BYTES_m11);
			uh = seg->metadata_fps->universal_header;
			if (uh->segment_UID == UID_NO_ENTRY_m11)
				generate_UID_m11(&uh->segment_UID);
			uh->segment_number = seg_num;
			snprintf_m11(seg->path, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", enclosing_path, seg->name, VIDEO_SEGMENT_DIRECTORY_TYPE_STRING_m11);
			snprintf_m11(seg->metadata_fps->full_file_name, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", seg->path, seg->name, VIDEO_METADATA_FILE_TYPE_STRING_m11);
			// video indices
			seg->video_indices_fps = FPS_allocate_processing_struct_m11(NULL, NULL, VIDEO_INDICES_FILE_TYPE_CODE_m11, VIDEO_INDEX_BYTES_m11, seg->metadata_fps, 0);
	...
	if (seg_recs == TRUE_m11) {
		...
		seg->record_indices_fps = FPS_allocate_processing_struct_m11(NULL, NULL, RECORD_INDICES_FILE_TYPE_CODE_m11, RECORD_INDEX_BYTES_m11, seg->metadata_fps, 0);
		snprintf_m11(seg->record_indices_fps->full_file_name, FULL_FILE_NAME_BYTES_m11, "%s/%s.%s", seg->path, seg->name, RECORD_INDICES_FILE_TYPE_STRING_m11);
	} else {
...
SESSION_m11	*allocate_session_m11(FILE_PROCESSING_STRUCT_m11 *proto_fps, si1 *enclosing_path, si1 *sess_name, si4 n_ts_chans, si4 n_vid_chans, si4 n_segs, si1 **ts_chan_names, si1 **vid_chan_names, TERN_m11 sess_recs, TERN_m11 segmented_sess_recs, TERN_m11 chan_recs, TERN_m11 seg_recs)
{
	si1                             free_names, number_str[FILE_NUMBERING_DIGITS_m11 + 1];
	si8                             i;
	SESSION_m11			*sess;
	UNIVERSAL_HEADER_m11		*uh;
	FILE_PROCESSING_STRUCT_m11	*gen_fps;
...
TERN_m11	encrypt_metadata_m11(FILE_PROCESSING_STRUCT_m11 *fps)
{
	...
	// section 2 encrypt
	if (md->section_1.section_2_encryption_level < NO_ENCRYPTION_m11) {  // natively encrypted and currently decrypted
		if (pwd->access_level >= -md->section_1.section_2_encryption_level) {
			...
			if (md->section_1.section_2_encryption_level == LEVEL_1_ENCRYPTION_m11)
				encryption_key = pwd->level_1_encryption_key;
			else
				encryption_key = pwd->level_2_encryption_key;
			encryption_blocks = METADATA_SECTION_2_BYTES_m11 / ENCRYPTION_BLOCK_BYTES_m11;
			ui1_p = (ui1 *) &md->section_2;
			for (i = encryption_blocks; i--;) {
				AES_encrypt_m11(ui1_p, ui1_p, NULL, encryption_key);
				ui1_p += ENCRYPTION_BLOCK_BYTES_m11;
			}
		}
	}
	...
	// section 3 encrypt
	if (md->section_1.section_3_encryption_level < NO_ENCRYPTION_m11) {  // natively encrypted and currently decrypted
		if (pwd->access_level >= -md->section_1.section_3_encryption_level) {
			...
			if (fps->metadata->section_1.section_3_encryption_level == LEVEL_1_ENCRYPTION_m11)
				...
				encryption_key = pwd->level_2_encryption_key;
			encryption_blocks = METADATA_SECTION_3_BYTES_m11 / ENCRYPTION_BLOCK_BYTES_m11;
...
TERN_m11     encrypt_time_series_data_m11(FILE_PROCESSING_STRUCT_m11 *fps)
{
	...
	if (pwd->access_level >= encryption_level) {
		if (encryption_level == LEVEL_1_ENCRYPTION_m11) {
			key = pwd->level_1_encryption_key;
			encryption_mask = CMP_BF_LEVEL_1_ENCRYPTION_MASK_m11;
		} else {
			key = pwd->level_2_encryption_key;
			encryption_mask = CMP_BF_LEVEL_2_ENCRYPTION_MASK_m11;
		}
	} else {
		error_message_m11("%s(): Cannot encrypt data => returning without encrypting\n", __FUNCTION__);
		fps->parameters.cps->directives.encryption_level = NO_ENCRYPTION_m11;
		return(FALSE_m11);
	}
	
	bh = (CMP_BLOCK_FIXED_HEADER_m11 *) fps->data_pointers;
	for (i = fps->number_of_items; i--;) {
		if (!(bh->block_flags & CMP_BF_ENCRYPTION_MASK_m11)) {  // already encrypted
			encryptable_blocks = (bh->total_block_bytes - CMP_BLOCK_ENCRYPTION_START_OFFSET_m11) / ENCRYPTION_BLOCK_BYTES_m11;
			if (bh->block_flags | CMP_BF_MBE_ENCODING_MASK_m11) {
				encryption_blocks = encryptable_blocks;
			} else {
			...
			ui1_p = (ui1 *) bh + CMP_BLOCK_ENCRYPTION_START_OFFSET_m11;
			for (j = encryption_blocks; j--;) {
				AES_encrypt_m11(ui1_p, ui1_p, NULL, key);
				ui1_p += ENCRYPTION_BLOCK_BYTES_m11;
			}
...
si1	**generate_numbered_names_m11(si1 **names, si1 *prefix, si4 number_of_names)
{
	...
	for (i = 0; i < number_of_names; ++i) {
		...
		snprintf_m11(names[i], SEGMENT_BASE_FILE_NAME_BYTES_m11, "%s%s", prefix, number_str);
	}
...
TERN_m11 generate_password_data_m11(FILE_PROCESSING_STRUCT_m11* fps, si1 *L1_pw, si1 *L2_pw, si1 *L3_pw, si1 *L1_pw_hint, si1 *L2_pw_hint)
{
	...
	si1			L1_pw_bytes[PASSWORD_BYTES_m11] = { 0 }, L2_pw_bytes[PASSWORD_BYTES_m11] = { 0 }, L3_pw_bytes[PASSWORD_BYTES_m11] = { 0 };
	si4			i;
	METADATA_SECTION_1_m11* md1;
	UNIVERSAL_HEADER_m11* uh;
	...
	if (L1_pw_hint != NULL)
		strncpy_m11(pwd->level_1_password_hint, L1_pw_hint, PASSWORD_HINT_BYTES_m11);
	if (L2_pw_hint != NULL)
		strncpy_m11(pwd->level_2_password_hint, L2_pw_hint, PASSWORD_HINT_BYTES_m11);
	...
	if (fps != NULL) {
		...
		if (uh->type_code == TIME_SERIES_METADATA_FILE_TYPE_CODE_m11 || uh->type_code == VIDEO_METADATA_FILE_TYPE_CODE_m11) {
			...
			if (L1_pw_hint != NULL)
				strncpy_m11(md1->level_1_password_hint, L1_pw_hint, PASSWORD_HINT_BYTES_m11);
			if (L2_pw_hint != NULL)
				strncpy_m11(md1->level_2_password_hint, L2_pw_hint, PASSWORD_HINT_BYTES_m11);
		}
	}

	// user passed level 1 password: generate validation field and encryption key
	if (check_password_m11(L1_pw) == FALSE_m11)
		return(FALSE_m11);
	...
	// get terminal bytes
	extract_terminal_password_bytes_m11(L1_pw, L1_pw_bytes);
	
	// generate Level 1 password validation field
	SHA_hash_m11((ui1 *) L1_pw_bytes, PASSWORD_BYTES_m11, hash);
	memcpy(uh->level_1_password_validation_field, hash, PASSWORD_VALIDATION_FIELD_BYTES_m11);
	if (globals_m11->verbose == TRUE_m11)
		message_m11("Level 1 password validation field generated");
	
	// generate encryption key
	AES_key_expansion_m11(pwd->level_1_encryption_key, L1_pw_bytes);
	if (globals_m11->verbose == TRUE_m11)
		message_m11("Level 1 encryption key generated");
	...
	if (L2_pw != NULL) {
		if (check_password_m11(L2_pw) == TRUE_m11) {
			
			// passed a level 2 password - level 2 access
			pwd->access_level = LEVEL_2_ACCESS_m11;
			
			// get terminal bytes
			extract_terminal_password_bytes_m11(L2_pw, L2_pw_bytes);
			
			// generate Level 2 password validation field
			SHA_hash_m11((ui1 *) L2_pw_bytes, PASSWORD_BYTES_m11, hash);
			memcpy(uh->level_2_password_validation_field, hash, PASSWORD_VALIDATION_FIELD_BYTES_m11);
			
			// exclusive or with level 1 password bytes
			for (i = 0; i < PASSWORD_VALIDATION_FIELD_BYTES_m11; ++i)
				uh->level_2_password_validation_field[i] ^= L1_pw_bytes[i];
			if (globals_m11->verbose == TRUE_m11)
				message_m11("Level 2 password validation field generated");
			
			// generate encryption key
			AES_key_expansion_m11(pwd->level_2_encryption_key, L2_pw_bytes);
			if (globals_m11->verbose == TRUE_m11)
				message_m11("Level 2 encryption key generated");
		} else {  // check_password_m11() == FALSE_m11
			return(FALSE_m11);
		}
	}
	...
	if (L3_pw != NULL) {
		if (check_password_m11(L3_pw) == TRUE_m11) {
			
			// get terminal bytes
			extract_terminal_password_bytes_m11(L3_pw, L3_pw_bytes);
			
			// generate Level 3 password validation field
			SHA_hash_m11((ui1 *) L3_pw_bytes, PASSWORD_BYTES_m11, hash);
			memcpy(uh->level_3_password_validation_field, hash, PASSWORD_VALIDATION_FIELD_BYTES_m11);
			if (pwd->access_level == LEVEL_1_ACCESS_m11) {  // only level 1 password passed
				for (i = 0; i < PASSWORD_VALIDATION_FIELD_BYTES_m11; ++i) // exclusive or with level 1 password bytes
					uh->level_3_password_validation_field[i] ^= L1_pw_bytes[i];
			} else {  // level 1 & level 2 passwords passed
				for (i = 0; i < PASSWORD_VALIDATION_FIELD_BYTES_m11; ++i) // exclusive or with level 2 password bytes
					uh->level_3_password_validation_field[i] ^= L2_pw_bytes[i];
			}
			if (globals_m11->verbose == TRUE_m11)
				message_m11("Level 3 password validation field generated");
		} else {  // check_password_m11() == FALSE_m11
			return(FALSE_m11);
		}
	}
...
void	initialize_metadata_m11(FILE_PROCESSING_STRUCT_m11 *fps, TERN_m11 initialize_for_update)
{
	...
	// section 1 fields
	md1->section_2_encryption_level = METADATA_SECTION_2_ENCRYPTION_LEVEL_DEFAULT_m11;
	md1->section_3_encryption_level = METADATA_SECTION_3_ENCRYPTION_LEVEL_DEFAULT_m11;
	md1->time_series_data_encryption_level = METADATA_TIME_SERIES_DATA_ENCRYPTION_LEVEL_DEFAULT_m11;
	
	// section 2 fields
	
	// type independent fields
	memset(tmd2->session_description, 0, METADATA_SESSION_DESCRIPTION_BYTES_m11);
	memset(tmd2->channel_description, 0, METADATA_CHANNEL_DESCRIPTION_BYTES_m11);
	memset(tmd2->segment_description, 0, METADATA_SEGMENT_DESCRIPTION_BYTES_m11);
	memset(tmd2->equipment_description, 0, METADATA_EQUIPMENT_DESCRIPTION_BYTES_m11);
	tmd2->acquisition_channel_number = METADATA_ACQUISITION_CHANNEL_NUMBER_NO_ENTRY_m11;
	
	// type specific fields
	switch (uh->type_code) {
		case TIME_SERIES_METADATA_FILE_TYPE_CODE_m11:
			tmd2->sampling_frequency = TIME_SERIES_METADATA_FREQUENCY_NO_ENTRY_m11;
			tmd2->low_frequency_filter_setting = TIME_SERIES_METADATA_FREQUENCY_NO_ENTRY_m11;
			tmd2->high_frequency_filter_setting = TIME_SERIES_METADATA_FREQUENCY_NO_ENTRY_m11;
			tmd2->notch_filter_frequency_setting = TIME_SERIES_METADATA_FREQUENCY_NO_ENTRY_m11;
			tmd2->AC_line_frequency = TIME_SERIES_METADATA_FREQUENCY_NO_ENTRY_m11;
			tmd2->amplitude_units_conversion_factor = TIME_SERIES_METADATA_AMPLITUDE_UNITS_CONVERSION_FACTOR_NO_ENTRY_m11;
			tmd2->time_base_units_conversion_factor = TIME_SERIES_METADATA_TIME_BASE_UNITS_CONVERSION_FACTOR_NO_ENTRY_m11;
			tmd2->absolute_start_sample_number = TIME_SERIES_METADATA_ABSOLUTE_START_SAMPLE_NUMBER_NO_ENTRY_m11;
			if (initialize_for_update == TRUE_m11) {
				...
				tmd2->number_of_samples = TIME_SERIES_METADATA_NUMBER_OF_SAMPLES_NO_ENTRY_m11;
				tmd2->number_of_blocks = TIME_SERIES_METADATA_NUMBER_OF_BLOCKS_NO_ENTRY_m11;
				tmd2->maximum_block_bytes = TIME_SERIES_METADATA_MAXIMUM_BLOCK_BYTES_NO_ENTRY_m11;
				tmd2->maximum_block_samples = TIME_SERIES_METADATA_MAXIMUM_BLOCK_SAMPLES_NO_ENTRY_m11;
				tmd2->maximum_block_keysample_bytes = TIME_SERIES_METADATA_MAXIMUM_BLOCK_KEYSAMPLE_BYTES_NO_ENTRY_m11;
				tmd2->maximum_block_duration = TIME_SERIES_METADATA_MAXIMUM_BLOCK_DURATION_NO_ENTRY_m11;
				tmd2->number_of_discontinuities = TIME_SERIES_METADATA_NUMBER_OF_DISCONTINUITIES_NO_ENTRY_m11;
				tmd2->maximum_contiguous_blocks = TIME_SERIES_METADATA_MAXIMUM_CONTIGUOUS_BLOCKS_NO_ENTRY_m11;
				tmd2->maximum_contiguous_block_bytes = TIME_SERIES_METADATA_MAXIMUM_CONTIGUOUS_BLOCK_BYTES_NO_ENTRY_m11;
				tmd2->maximum_contiguous_samples = TIME_SERIES_METADATA_MAXIMUM_CONTIGUOUS_SAMPLES_NO_ENTRY_m11;
			}
			break;
		case VIDEO_METADATA_FILE_TYPE_CODE_m11:
			vmd2->time_base_units_conversion_factor = VIDEO_METADATA_TIME_BASE_UNITS_CONVERSION_FACTOR_NO_ENTRY_m11;
			vmd2->absolute_start_frame_number = VIDEO_METADATA_ABSOLUTE_START_FRAME_NUMBER_NO_ENTRY_m11;
			vmd2->frame_rate = VIDEO_METADATA_FRAME_RATE_NO_ENTRY_m11;
			vmd2->horizontal_pixels = VIDEO_METADATA_HORIZONTAL_PIXELS_NO_ENTRY_m11;
			vmd2->vertical_pixels = VIDEO_METADATA_VERTICAL_PIXELS_NO_ENTRY_m11;
			if (initialize_for_update == TRUE_m11) {
				...
				vmd2->number_of_frames = VIDEO_METADATA_NUMBER_OF_FRAMES_NO_ENTRY_m11;
				vmd2->number_of_clips = VIDEO_METADATA_NUMBER_OF_CLIPS_NO_ENTRY_m11;
				vmd2->maximum_clip_bytes = VIDEO_METADATA_MAXIMUM_CLIP_BYTES_NO_ENTRY_m11;
				vmd2->maximum_clip_frames = VIDEO_METADATA_MAXIMUM_CLIP_FRAMES_NO_ENTRY_m11;
				vmd2->number_of_video_files = VIDEO_METADATA_NUMBER_OF_VIDEO_FILES_NO_ENTRY_m11;
				vmd2->maximum_clip_duration = VIDEO_METADATA_MAXIMUM_CLIP_DURATION_NO_ENTRY_m11;
				vmd2->number_of_discontinuities = VIDEO_METADATA_NUMBER_OF_DISCONTINUITIES_NO_ENTRY_m11;
				vmd2->maximum_contiguous_clips = VIDEO_METADATA_MAXIMUM_CONTIGUOUS_CLIPS_NO_ENTRY_m11;
				vmd2->maximum_contiguous_clip_bytes = VIDEO_METADATA_MAXIMUM_CONTIGUOUS_CLIP_BYTES_NO_ENTRY_m11;
				vmd2->maximum_contiguous_frames = VIDEO_METADATA_MAXIMUM_CONTIGUOUS_FRAMES_NO_ENTRY_m11;
			}
			break;
		default:
			error_message_m11("%s(): Unrecognized METADATA SECTION 2 type in file \"%s\"\n", __FUNCTION__, fps->full_file_name);
			break;
	}
	
	// section 3 fields
	md3->recording_time_offset = globals_m11->recording_time_offset;
	md3->daylight_time_start_code = globals_m11->daylight_time_start_code;
	md3->daylight_time_end_code = globals_m11->daylight_time_end_code;
	strncpy_m11(md3->standard_timezone_acronym, globals_m11->standard_timezone_acronym, TIMEZONE_ACRONYM_BYTES_m11);
	strncpy_m11(md3->standard_timezone_string, globals_m11->standard_timezone_string, TIMEZONE_STRING_BYTES_m11);
	strncpy_m11(md3->daylight_timezone_acronym, globals_m11->daylight_timezone_acronym, TIMEZONE_ACRONYM_BYTES_m11);
	strncpy_m11(md3->daylight_timezone_string, globals_m11->daylight_timezone_string, TIMEZONE_STRING_BYTES_m11);
	memset(md3->subject_name_1, 0, METADATA_SUBJECT_NAME_BYTES_m11);
	memset(md3->subject_name_2, 0, METADATA_SUBJECT_NAME_BYTES_m11);
	memset(md3->subject_name_3, 0, METADATA_SUBJECT_NAME_BYTES_m11);
	memset(md3->subject_ID, 0, METADATA_SUBJECT_ID_BYTES_m11);
	memset(md3->recording_country, 0, METADATA_RECORDING_LOCATION_BYTES_m11);
	memset(md3->recording_territory, 0, METADATA_RECORDING_LOCATION_BYTES_m11);
	memset(md3->recording_locality, 0, METADATA_RECORDING_LOCATION_BYTES_m11);
	memset(md3->recording_institution, 0, METADATA_RECORDING_LOCATION_BYTES_m11);
	memset(md3->geotag_format, 0, METADATA_GEOTAG_FORMAT_BYTES_m11);
	memset(md3->geotag_data, 0, METADATA_GEOTAG_DATA_BYTES_m11);
	md3->standard_UTC_offset = globals_m11->standard_UTC_offset;
...
void	lh_set_directives_m11(si1 *full_file_name, ui8 lh_flags, TERN_m11 *mmap_flag, TERN_m11 *close_flag, si8 *number_of_items)
{
	...
	if ((lh_flags & (LH_ALL_READ_FLAGS_m11 | LH_ALL_MEM_MAP_FLAGS_m11)) == 0)
		return;
	...
	// segment data types
		...
		switch (type_code) {
			case TIME_SERIES_INDICES_FILE_TYPE_CODE_m11:
			case VIDEO_INDICES_FILE_TYPE_CODE_m11:
				if (lh_flags & LH_READ_SEGMENT_DATA_m11) {
					*number_of_items = FPS_FULL_FILE_m11;
					*close_flag = TRUE_m11;
					*mmap_flag = FALSE_m11;
				}
...
void    reset_metadata_for_update_m11(FILE_PROCESSING_STRUCT_m11 *fps)
{
	...
	switch (fps->universal_header->type_code) {
		...
		case VIDEO_METADATA_FILE_TYPE_CODE_m11:
			...
			break;
		default:
			error_message_m11("%s(): Unrecognized metadata type in file \"%s\"\n", __FUNCTION__, fps->full_file_name);
			break;
	}
...
void    show_location_info_m11(LOCATION_INFO_m11 *li)
{
	...
	printf_m11("Timezone Description: %s\n", li->timezone_description);
	printf_m11("Latitude: %lf\n", li->latitude);
	printf_m11("Longitude: %lf\n", li->longitude);
	printf_m11("WAN_IPv4 Address: %s\n", li->WAN_IPv4_address);
...
void    show_timezone_info_m11(TIMEZONE_INFO_m11 *timezone_entry, TERN_m11 show_DST_detail)
{
	...
	if (timezone_entry->daylight_time_start_code && show_DST_detail == TRUE_m11) {
		if (timezone_entry->daylight_time_start_code == DTCC_VALUE_NO_ENTRY_m11) {
			printf_m11("Daylight Time info not available\n");
		}
		else {
		...
		printf_m11("Daylight Time not observed\n");
	}
	return;
}
...
si8	write_file_m11(FILE_PROCESSING_STRUCT_m11 *fps, si8 file_offset, si8 bytes_to_write, si8 number_of_items, void *external_data, ui4 behavior_on_fail)
{
	...
	switch (uh->type_code) {
		case TIME_SERIES_DATA_FILE_TYPE_CODE_m11:
			encrypt_time_series_data_m11(fps);
			break;
		case RECORD_DATA_FILE_TYPE_CODE_m11:
			encrypt_record_data_m11(fps);
			break;
		case TIME_SERIES_METADATA_FILE_TYPE_CODE_m11:
		case VIDEO_METADATA_FILE_TYPE_CODE_m11:
			encrypt_metadata_m11(fps);
	...
	// Calculate CRCs
	// IMPORTANT: if the file is written non-sequentially (not FPS_APPEND_m11 or FPS_FULL_FILE_m11), the CRCs will be invalid
	if (globals_m11->CRC_mode & (CRC_CALCULATE_m11 | CRC_CALCULATE_ON_OUTPUT_m11)) {
		switch (uh->type_code) {
			case TIME_SERIES_DATA_FILE_TYPE_CODE_m11:
				calculate_time_series_data_CRCs_m11(fps);
				break;
			case RECORD_DATA_FILE_TYPE_CODE_m11:
				calculate_record_data_CRCs_m11(fps);
				break;
			case TIME_SERIES_INDICES_FILE_TYPE_CODE_m11:
			case VIDEO_INDICES_FILE_TYPE_CODE_m11:
			case RECORD_INDICES_FILE_TYPE_CODE_m11:
				calculate_indices_CRCs_m11(fps);
				break;
			case TIME_SERIES_METADATA_FILE_TYPE_CODE_m11:
			case VIDEO_METADATA_FILE_TYPE_CODE_m11:
				calculate_metadata_CRC_m11(fps);
				break;
		}
	}
...
void	AES_encrypt_m11(ui1 *in, ui1 *out, si1 *password, ui1 *expanded_key)
{
	...
	if (expanded_key != NULL) {
		AES_cipher_m11(in, out, state, expanded_key);
	}
	else if (password != NULL) {
		// password becomes the key (16 bytes, zero-padded if shorter, truncated if longer)
		strncpy_m11(key, password, 16);
		...
		AES_cipher_m11(in, out, state, round_key);
	}
	else {
		error_message_m11("%s(): No password or expanded key\n", __FUNCTION__);
	}
...
void	AES_cipher_m11(ui1 *in, ui1 *out, ui1 state[][4], ui1 *round_key)
{
	...
	for (round = 1; round < AES_NR_m11; round++) {
		AES_sub_bytes_m11(state);
		AES_shift_rows_m11(state);
		AES_mix_columns_m11(state);
...
void	AES_sub_bytes_m11(ui1 state[][4])
{
	...
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			state[i][j] = (ui1)AES_get_sbox_value_m11(state[i][j]);
		}
	}
...
void    CMP_calculate_statistics_m11(REC_Stat_v10_m11 *stats, si4 *input_buffer, si8 len, CMP_NODE_m11 *nodes)
{
		...
		if (median_found == 0) {
			...
			if (running_cnt >= mid_idx) {
				if (running_cnt == mid_idx) {
					...
					stats->median = CMP_round_si4_m11(true_median);
				}
				else {
					stats->median = np->val;
				}
...
ui1	CMP_differentiate_m11(CMP_PROCESSING_STRUCT_m11 *cps)
{
	...
	if (cps->directives.find_derivative_level == TRUE_m11) {
		...
		if (cps->parameters.scrap_buffers == NULL)
			cps->parameters.scrap_buffers = CMP_allocate_buffers_m11(cps->parameters.scrap_buffers, 1, n_samps, sizeof(si4), FALSE_m11, FALSE_m11);
	} else if (cps->directives.set_derivative_level == TRUE_m11) {
		set_deriv_level = cps->parameters.goal_derivative_level;
	}
	if (set_deriv_level == 0) {
		warning_message_m11("%s(): requested derivative level is zero\n", __FUNCTION__);
		...
		cps->parameters.minimum_difference_value = cps->parameters.maximum_difference_value = 0;
		return(0);
	}
	...
	pos_inf_si4 = (si8) POS_INF_SI4_m11;
	neg_inf_si4 = (si8) NEG_INF_SI4_m11;
	for (i = n_diffs; i--;) {
		if (*si4_p1 < samp_min)
			samp_min = *si4_p1;
	...
	while (--n_diffs) {
		...
		*si4_p3 = *si4_p1;  // derivative initial value
		...
		if (range < last_range) {
			...
			cps->parameters.derivative_level = deriv_level;
			last_range = range;
			if (set_deriv_level == 0xFF)  // find derivative level not done in place
				memcpy((void *) input_buffer, (void *) deriv_buffer, (size_t) (n_samps << 2));
		} else {
			--deriv_level;
			break;
		}
...
TERN_m11     CMP_encrypt_m11(FILE_PROCESSING_STRUCT_m11 *fps)
{
	ui1				*ui1_p, *key;
	si1				encryption_level;
	...
	block_header = cps->block_header;
	encryption_bits = block_header->block_flags & CMP_BF_ENCRYPTION_MASK_m11;
	
	// check if block is already encrypted
	switch (encryption_bits) {
		case 0: // not encrypted
			break;
		case CMP_BF_ENCRYPTION_MASK_m11:
			warning_message_m11("%s(): Level 1 & 2 bits set in block => cannot encrypt\n", __FUNCTION__);
			return(FALSE_m11);
		case CMP_BF_LEVEL_1_ENCRYPTION_MASK_m11:
			if (encryption_level == LEVEL_1_ENCRYPTION_m11)
				return(TRUE_m11); // already encrypted
			CMP_decrypt_m11(fps); // encrypted, but at wrong level
			break;
		case CMP_BF_LEVEL_2_ENCRYPTION_MASK_m11:
			if (encryption_level == LEVEL_2_ENCRYPTION_m11)
				return(TRUE_m11); // already encrypted
			CMP_decrypt_m11(fps); // encrypted, but at wrong level
			break;
	}
	...
	if (pwd->access_level >= encryption_level) {
		...
		error_message_m11("%s(): Cannot encrypt data => insufficient access\n", __FUNCTION__);
		return(FALSE_m11);
	}
...
void    CMP_find_extrema_m11(si4 *input_buffer, si8 len, si4 *minimum, si4 *maximum, CMP_PROCESSING_STRUCT_m11 *cps)
{
	si4     min, max;
	si8     i;
...
void    CMP_MBE_encode_m11(CMP_PROCESSING_STRUCT_m11 *cps)
{
	...
	block_header = cps->block_header;
	MBE_header = (CMP_MBE_MODEL_FIXED_HEADER_m11 *) cps->parameters.model_region;
	if (MBE_header->flags & CMP_MBE_FLAGS_PREPROCESSED_MASK_m11) {
	...
	n_samps = block_header->number_of_samples;
	if (n_derivs) {
		init_out_vals = (si4 *) (cps->parameters.model_region + CMP_MBE_MODEL_FIXED_HEADER_BYTES_m11);
		for (i = 0; i < n_derivs; ++i)
	...
	block_header->model_region_bytes = CMP_MBE_MODEL_FIXED_HEADER_BYTES_m11 + (n_derivs * 4);
	...
	block_header->total_block_bytes = pad_m11((ui1 *) block_header, (si8) (cmp_data_bytes + block_header->total_header_bytes), 8);
...
{
	TERN_m11	no_zero_counts, use_raw;
	...
	ui4		cat_total_counts[CMP_PRED_CATS_m11], goal_total_counts[CMP_PRED_CATS_m11], bin, fall_through_bytes;
	...
	CMP_STATISTICS_BIN_m11			**sorted_count, temp_sorted_count;
	CMP_BLOCK_FIXED_HEADER_m11		*block_header;
	CMP_PRED_MODEL_FIXED_HEADER_m11		*PRED_header;
	CMP_MBE_MODEL_FIXED_HEADER_m11		*MBE_header;
	...
	if (n_samps <= 1) {
		block_header->model_region_bytes = (ui2) CMP_PRED_MODEL_FIXED_HEADER_BYTES_m11;
		if (block_header->number_of_samples == 1) {
		...
		block_header->total_block_bytes = pad_m11((ui1 *) block_header, block_header->total_header_bytes, 8);
		return;
	}
	...
	cumulative_count = (ui8 **) cps->parameters.cumulative_count;
	minimum_range = (ui8 **) cps->parameters.minimum_range;
	sorted_count = (CMP_STATISTICS_BIN_m11 **) cps->parameters.sorted_count;
	...
	no_zero_counts = FALSE_m11;
	if (cps->directives.no_zero_counts == TRUE_m11) {
		no_zero_counts = TRUE_m11;
		PRED_header->flags |= CMP_PRED_FLAGS_NO_ZERO_COUNTS_MASK_m11;
	}
	overflow_bytes = CMP_get_overflow_bytes_m11(cps, CMP_COMPRESSION_MODE_m11, CMP_PRED_COMPRESSION_m11);
	...
	for (i = n_deriv_samps; i--;) {
		...
		if (diff < -127 || diff > 127) {
			ui1_p = (ui1 *) &diff;
			++count[prev_cat][*key_p++ = CMP_UI1_KEYSAMPLE_FLAG_m11];
			prev_cat = CMP_PRED_NEG_m11;
			j = overflow_bytes; do {
				++count[prev_cat][*key_p = *ui1_p++];
				prev_cat = CMP_PRED_CAT_m11(*key_p); ++key_p;  // do not increment within call to CAT
			} while (--j);
		} else {
	...
	for (i = 0; i < CMP_PRED_CATS_m11; ++i) {
		goal_total_counts[i] = CMP_RED_TOTAL_COUNTS_m11 - 1;
		if (no_zero_counts == TRUE_m11)
	...
	for (i = n_keysamp_bytes; i;) {
		for (; range >= minimum_range[prev_cat][bin = symbol_map[prev_cat][*key_p]]; key_p++) {
			...
			if (!--i)
				break;
			prev_cat = CMP_PRED_CAT_m11(*key_p);
		}
	...
	block_header->total_block_bytes = (ui4) pad_m11((ui1 *) block_header, PRED_total_bytes, 8);
	
	// calculate fall through encoding bytes
	if (cps->directives.fall_through_to_best_encoding == TRUE_m11) {
		...
		fall_through_bytes = MBE_data_bits >> 3;
		if (MBE_data_bits & 7)
			++fall_through_bytes;
		header_bytes = cps->parameters.model_region - (ui1 *) block_header;  // fixed block bytes + variable region before model
		fall_through_bytes += header_bytes + CMP_MBE_MODEL_FIXED_HEADER_BYTES_m11 + (n_derivs * 4);
		fall_through_bytes += (fall_through_bytes & 7);  // pad bytes
		if (fall_through_bytes < block_header->total_block_bytes) {
			...
			MBE_header->bits_per_sample = bits_per_samp;
			MBE_header->flags = CMP_MBE_FLAGS_PREPROCESSED_MASK_m11;
...
CMP_PROCESSING_STRUCT_m11	*CMP_reallocate_processing_struct_m11(FILE_PROCESSING_STRUCT_m11 *fps, ui4 mode, si8 data_samples, ui4 block_samples)
{
	...
	switch (mode) {
		case CMP_COMPRESSION_MODE_m11:
			...
			if (cps->parameters.allocated_compressed_bytes) {  // none may be have been allocated by CMP_allocate_processing_struct_m11() - if so, can't check
			...
			if (cps->parameters.allocated_decompressed_samples) {  // none may be have been allocated by CMP_allocate_processing_struct_m11() - if so, can't check, but don't realloc
	...
	if (fps->time_series_data != NULL && new_compressed_bytes) // FPS_reallocate_processing_struct_m11() resets cps->block_header for time series data fps
...
{
	TERN_m11	pos_derivs, no_zero_counts, use_raw;
	...
	// zero or one or samples
	if (n_samps <= 1) {
		block_header->model_region_bytes = (ui2) CMP_RED_MODEL_FIXED_HEADER_BYTES_m11;
		if (block_header->number_of_samples == 1) {
			*((si4 *) (cps->parameters.model_region + block_header->model_region_bytes)) = cps->input_buffer[0];  // note: no statistics
			block_header->model_region_bytes += sizeof(si4);
		}
	...
	if (n_derivs && cps->parameters.minimum_difference_value > 0) {
		pos_derivs = TRUE_m11;
		RED_header->flags |= CMP_RED_FLAGS_POSITIVE_DERIVATIVES_MASK_m11;
	}
	no_zero_counts = FALSE_m11;
	if (cps->directives.no_zero_counts == TRUE_m11) {
		no_zero_counts = TRUE_m11;
		RED_header->flags |= CMP_RED_FLAGS_NO_ZERO_COUNTS_MASK_m11;
	}
	overflow_bytes = CMP_get_overflow_bytes_m11(cps, CMP_COMPRESSION_MODE_m11, CMP_RED_COMPRESSION_m11);

	// generate count & build keysample array
	if (pos_derivs == TRUE_m11) {
		low_d = 1; high_d = 255;
		ks_flag = CMP_POS_DERIV_KEYSAMPLE_FLAG_m11;  // == 0 (non-overflow range: 1 to 255)
	} else {
		low_d = -127; high_d = 127;
		ks_flag = CMP_UI1_KEYSAMPLE_FLAG_m11;  // == -128 (non-overflow range: -127 to +127)
	}
	memset((void *) count, 0, CMP_RED_MAX_STATS_BINS_m11 * sizeof(ui4));
	...
	} else {  // pos_derivs == FALSE_m11: interleave (to speed up sort)
	...
	goal_total_counts = CMP_RED_TOTAL_COUNTS_m11 - 1;
	if (no_zero_counts == TRUE_m11) {
	...
	block_header->total_block_bytes = (ui4) pad_m11((ui1 *) block_header, RED_total_bytes, 8);
...
void    CMP_scale_amplitude_si4_m11(si4 *input_buffer, si4 *output_buffer, si8 len, sf8 scale_factor, CMP_PROCESSING_STRUCT_m11 *cps)
{
	...
	if (cps != NULL) {
		...
		// store
		*((sf4 *) cps->block_parameters + cps->parameters.block_parameter_map[CMP_PF_AMPLITUDE_SCALE_IDX_m11]) = scale_factor;
	...
	while (len--)
		*si4_p2++ = CMP_round_si4_m11((sf8) *si4_p1++ * inv_scale_factor);
...
void    CMP_scale_frequency_si4_m11(si4 *input_buffer, si4 *output_buffer, si8 len, sf8 scale_factor, CMP_PROCESSING_STRUCT_m11 *cps)
{
	...
	if (cps != NULL) {
		...
		sf4_scale = (sf4) scale_factor;
		// store
		*((sf4 *) cps->block_parameters + cps->parameters.block_parameter_map[CMP_PF_FREQUENCY_SCALE_IDX_m11]) = scale_factor;
		// promote back to sf8 (having only sf4 precision)
		scale_factor = (sf8) sf4_scale;
	}
...
void    CMP_set_variable_region_m11(CMP_PROCESSING_STRUCT_m11 *cps)
{
	...
	// records region
	var_reg_ptr = (ui1 *) block_header + CMP_BLOCK_FIXED_HEADER_BYTES_m11;  // pointer to start of variable region
	cps->block_records = var_reg_ptr;
	var_reg_ptr += block_header->record_region_bytes;
	...
	if (cps->directives.detrend_data == TRUE_m11)
		block_header->parameter_flags |= (CMP_PF_INTERCEPT_MASK_m11 | CMP_PF_GRADIENT_MASK_m11);
	else
		block_header->parameter_flags &= ~(CMP_PF_INTERCEPT_MASK_m11 | CMP_PF_GRADIENT_MASK_m11);
	
	if (cps->directives.set_amplitude_scale == TRUE_m11 || cps->directives.find_amplitude_scale == TRUE_m11)
		block_header->parameter_flags |= CMP_PF_AMPLITUDE_SCALE_MASK_m11;
	else
		block_header->parameter_flags &= ~CMP_PF_AMPLITUDE_SCALE_MASK_m11;
	
	if (cps->directives.set_frequency_scale == TRUE_m11 || cps->directives.find_frequency_scale == TRUE_m11)
		block_header->parameter_flags |= CMP_PF_FREQUENCY_SCALE_MASK_m11;
	else
		block_header->parameter_flags &= ~CMP_PF_FREQUENCY_SCALE_MASK_m11;
	
	if (cps->directives.include_noise_scores == TRUE_m11)
		block_header->parameter_flags |= CMP_PF_NOISE_SCORES_MASK_m11;
	else
		block_header->parameter_flags &= ~CMP_PF_NOISE_SCORES_MASK_m11;
...
{
	TERN_m11        free_nodes;
	CMP_NODE_m11	*last_node, *next_node, *prev_node, *np;
	si8             i, j, n_nodes;
	sf8             new_val;
	si4		*sorted_x;
	va_list         args;
	...
	np = nodes;
	head->next = tail;
	head->val = (si4) NAN_SI4_m11;  // This is 0x80000000, but NEG_INF_SI4_m11 (0x80000001) could theoretically fail
	tail->val = (si4) POS_INF_SI4_m11;  // This is 0x7FFFFFFF
	tail->prev = head;
	...
	if (return_sorted_ts == TRUE_m11) {
		va_start(args, return_sorted_ts);
		sorted_x = va_arg(args, si4 *);
		va_end(args);
		if (sorted_x == NULL) {
			warning_message_m11("%s(): passed sorted array pointer is NULL\n", __FUNCTION__);
		} else {
			for (i = n_nodes, np = head->next; i--; np = np->next)
				for (j = np->count; j--;)
					*sorted_x++ = np->val;
...
si8	FPS_write_m11(FILE_PROCESSING_STRUCT_m11 *fps, si8 file_offset, si8 bytes_to_write, const si1 *function, si4 line, ui4 behavior_on_fail)
{
	...
	// flush
	if (fps->directives.flush_after_write == TRUE_m11 || fps->directives.update_universal_header == TRUE_m11)
