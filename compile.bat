...
::			libmed_m10_win.lib (file)
::	MED2RAW (dir)
::		Windows (dir)
::			MED2RAW.c (file)
::
:: Link the library with "/link %LIBOBJ%\libmed_m10%LIBSFX%.lib" in executables, as below.
::
...
cl /I%LIBINC% /I%PRGINC% /I%TGTINC% /Gd /GL /EHsc /nologo /O2 /D NDEBUG /D _CONSOLE /D UNICODE /D _UNICODE %PRGSRC%\%PRG%.c /link /MACHINE:X64 %LIBOBJ%\libmed_m10%LIBSFX%.lib /OUT:%PRGOBJ%\%PRG%.exe
